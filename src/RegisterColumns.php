...
/**
 * Check if the class `RegisterColumns` is defined, and if not, define it.
 */
if ( ! class_exists( 'RegisterColumns' ) ):
		...
		public function register_columns( array $columns ): array {
			...
			foreach ( $custom_columns as $key => $column ) {
				...
				if ( str_starts_with( $position, 'after:' ) ) {
					$columns = Utils::insert_after( $columns, $reference_column, [ $key => $label ] );
